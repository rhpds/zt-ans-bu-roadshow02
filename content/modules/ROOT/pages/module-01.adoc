...
First, log into the Ansible Automation Platform instance using the credentials below. Your DNS domain for this lab is **lab.{dns_domain}**.
...
|===
| Server | FQDN
| Domain Controller | `windows.lab.{dns_domain}`
| Dev Windows Server | `dbserver.lab.{dns_domain}`
|===
...
+
Once the workflow is complete (after 3-4 minutes), click on the **Windows Server Applications** node and review the job output. You can verify that packages like `windirstat` and `7zip` were installed.
+
...
+
NOTE: This process will take several minutes to deploy Active Directory and reboot the system. You can proceed to deploy the RHEL node. You will need the Active Directory for the next module.
...
If you have encountered an issue or have noticed something not quite right, please link:https://github.com/ansible/workshops/issues/new?labels=controller-101&title=Issue+with+Roadshow02&assignees=nmartins0611[open an issue on GitHub].
