...
image::ticket04.png[Support ticket for web app deployment, opts="border"]
...
+
image::win-webapp.png[Web application running on the Windows server, opts="border"]
...
+
image::rhel-webapp.png[Web application running on the RHEL server, opts="border"]
...
+
image::loadbalancer.png[Default HAProxy statistics page, opts="border"]
...
+
`node01.lab.{dns_domain}`
+
image::hostname.png[Entering a hostname in the survey prompt, opts="border"]
+
Launch the job.
...
- name: clone a git repo
  ansible.builtin.git:
    repo: https://github.com/rhpds/summit-2026-nmartins0611-aap25-roadshow-content.git
    dest: /tmp/repo
